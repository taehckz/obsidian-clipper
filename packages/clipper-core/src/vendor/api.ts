...
function fallbackMarkdownFromHtml(html: string): string {
	...
	const doc = parseHTML(wrapped).document;
...
export async function clip(options: ClipOptions): Promise<ClipResult> {
	...
	// Use the full document object. Some sites (e.g., docs-style pages) lose content
	// when only documentElement is passed to Defuddle.
	const defuddle = new DefuddleClass(doc as unknown as Document, { url });
	const defuddleResult = await defuddle.parseAsync();

	// Convert to markdown, with a local fallback for environments where
	// defuddle/full cannot access full browser-like DOM globals.
	let markdownContent = createMarkdownContent(defuddleResult.content, url);
	if (typeof markdownContent !== 'string' ||
		markdownContent.startsWith('Partial conversion completed with errors.')) {
		markdownContent = fallbackMarkdownFromHtml(defuddleResult.content || '');
	}
