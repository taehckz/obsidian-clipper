...
async function resolveSelectorVariable(selectorExpr: string, tabId?: number): Promise<any> {
	// In package mode, selector resolution is expected via custom resolver paths.
	// Keep this fallback non-throwing for compatibility.
	console.warn('Selector resolution fallback is unavailable in package mode:', selectorExpr, tabId);
	return undefined;
}
